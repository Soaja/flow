...
export default function Manifesto() {
  ...
  useGsapIdle(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.from('.flow-poster', { x: 52, opacity: 0, rotation: 2, duration: .9, ease: 'power3.out', scrollTrigger: { trigger: sectionRef.current, start: 'top 60%' } });
    }, sectionRef);
    return () => ctx.revert();
  });
    ...
    <section id="manifesto" ref={sectionRef} className="about-screen">
      ...
      <div className="about-layout">
        <div className="about-main-copy">
          ...
          <h2><strong>A sports marketing and media agency</strong> giving sport a new voice.</h2>
        ...
        <div className="flow-poster" aria-label="FLOW brand mark">
          <div className="poster-grain" />
          <span className="poster-top">FLOW / BELGRADE</span>
          <img className="poster-logo" src="/flow-logo.svg" alt="FLOW" />
          <div className="poster-pulse"><span /><img src="/flow-symbol.svg" alt="" /></div>
          <p>Future communication<br />of sports.</p>
          <span className="poster-bottom">EUROPE — WORLDWIDE</span>
        </div>
      </div>
      ...
      <style>{`
        ...
        .about-layout{width:min(1400px,100%);margin:auto;display:grid;grid-template-columns:minmax(0,1.55fr) minmax(300px,.65fr);gap:clamp(44px,6vw,100px);align-items:center;transform:translateY(-42px)}
        .about-main-copy{max-width:870px}
        .about-label{display:flex;align-items:center;gap:13px;margin-bottom:22px;font-size:.68rem;font-weight:700;letter-spacing:.18em;text-transform:uppercase;color:var(--accent)}
        .about-label span{display:grid;place-items:center;width:32px;height:32px;border:1px solid rgba(202,219,46,.45);border-radius:50%;font-size:.55rem}
        .about-main-copy h2{max-width:900px;font-family:var(--f-display);font-size:clamp(2.5rem,4.2vw,5rem);font-weight:700;line-height:.98;letter-spacing:-.045em;text-transform:uppercase;color:var(--fg)}
        .about-main-copy h2 strong{color:var(--accent);font-weight:700}
        .about-intro{margin-top:20px;font-family:var(--f-display);font-size:clamp(1rem,1.35vw,1.3rem);font-weight:700;text-transform:uppercase;letter-spacing:.025em;color:var(--fg)}
        .about-description{max-width:720px;margin-top:13px;font-size:clamp(.78rem,.92vw,.94rem);line-height:1.6;color:rgba(231,233,234,.68)}
        .about-main-copy h3{margin-top:28px;font-family:var(--f-display);font-size:.87rem;font-weight:700;letter-spacing:.04em;text-transform:uppercase;color:var(--fg)}
        .service-cloud{display:flex;flex-wrap:wrap;gap:8px;margin-top:14px;max-width:850px}
        .service-chip{display:flex;align-items:center;gap:10px;min-height:42px;padding:8px 16px;border:1px solid rgba(231,233,234,.18);border-radius:999px;background:rgba(231,233,234,.055);backdrop-filter:blur(12px);font-size:clamp(.68rem,.82vw,.82rem);font-weight:600;color:var(--fg);transition:background .25s,border-color .25s,color .25s,transform .25s}
        ...
        .moments-dock{position:absolute;left:92px;right:0;bottom:0;height:190px;display:grid;grid-template-columns:240px 1fr;background:#0d0e0d;border-top:1px solid rgba(202,219,46,.4);z-index:3}
